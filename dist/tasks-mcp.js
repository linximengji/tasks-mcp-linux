    ...
    try {
        switch (name) {
            ...
            case 'tasks_mark_done': {
                ...
                const index = load();
                if (!index[taskId])
                    return { content: [{ type: 'text', text: JSON.stringify({ error: `task not found: ${taskId}` }) }], isError: true };
            ...
            case 'tasks_set_progress': {
                ...
                if (!index[taskId])
                    return { content: [{ type: 'text', text: JSON.stringify({ error: `task not found: ${taskId}` }) }], isError: true };
                if (start && index[taskId].status === 'pending')
                    index[taskId].status = 'in_progress';
...
function jsonRpc(id, result, error) {
    ...
    if (error)
        msg.error = { code: error.code || -32603, message: error.message || 'Internal error' };
    else
        msg.result = result;
...
//# sourceMappingURL=tasks-mcp.js.map